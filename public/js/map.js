...
import { tr, LEVEL_NAME } from './i18n.js?v=2.4.29'
import { createOsmBuildingsLayer } from './layers/osm-buildings.js?v=2.4.29'
import { createProvinceBoundariesLayer } from './layers/province-boundaries.js?v=2.4.29'
import { createSatelliteLayers, ensureMapPanes, LAYER_GROUPS, allLayerToggles, createBurnScarLayer } from './layers/satellite.js?v=2.4.29'
import { createBasemaps, BASEMAP_META } from './layers/basemaps.js?v=2.4.29'
import { createPm25HeatmapLayer } from './layers/pm25-heatmap.js?v=2.4.29'
import { createNewsFireLayer } from './layers/news-fire.js?v=2.4.29'
import { createDroughtLayer } from './layers/drought.js?v=2.4.29'
import { paintRisk, paintAir, paintRain, pm25Color } from './paint.js?v=2.4.29'
...
export function initMap() {
  ...
  // Isolated in its own try/catch: drought is one optional overlay among
  // many, and initMap() itself is a single point of failure for the
  // entire dashboard (see the 2026-09-15/16 outage note in main.js) — a
  // problem in this one layer must not cost the user the map, the risk
  // shading, or every other layer initialized above and below this line.
  try {
  ...
  // reload. refresh() is async; an un-caught rejection here is an
  // unhandled-promise-rejection with nothing downstream to catch it —
  // .catch() makes a fetch/parse failure a logged non-event instead.
  droughtApi?.refresh().catch((e) => console.error('drought refresh failed:', e))
  on('snapshot', () => droughtApi?.refresh().catch((e) => console.error('drought refresh failed:', e)))
  return map
}
...
function addLegend() {
  ...
  legendCtl.onAdd = () => {
    ...
    body.innerHTML = `
      ...
      <div class="lrow"><span class="lsw" style="background:linear-gradient(90deg,#f7f4b8,#f0a030,#c0341a,#6b1f10)"></span>${tr('AOD 3 กม. (MODIS Aqua — ละเอียดกว่า AOD ด้านบน 50 เท่า)', 'AOD 3 km (MODIS Aqua — 50× sharper than the AOD above)')}</div>
